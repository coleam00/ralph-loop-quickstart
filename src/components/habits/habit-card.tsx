...
export function HabitCard({
  ...
  streak = 0,
  milestone = null,
  milestoneColor = 'text-muted-foreground',
  onEdit,
  onDelete,
  onCompletionChange
}: HabitCardProps) {
      ...
      <CardContent>
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className={`font-medium ${streak > 0 ? '' : 'text-muted-foreground'}`}>
              {streak} {habit.frequency === 'daily' ? 'day' : 'week'} streak
            </span>
            {milestone && (
              <span className={`text-xs px-2 py-0.5 rounded-full bg-secondary ${milestoneColor}`}>
                {milestone}
              </span>
            )}
          </div>
